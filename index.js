...
function generateRandomPassword(length) {
 let password=""; // Створюємо порожній рядок для збереження паролю.
  let characters="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";// Створюємо рядок characters "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789" з доступних символів для паролю.
  for(let i=0;i<characters.length;i++)// За допомогою циклу for проходимось по кожному символу рядка characters
  {let x;
   do{x=Math.ceil(Math.random()*100);}while(x>characters.length);
   if(password.length<length){

    password=password+characters[x];}};// Визначаємо випадковий індекс символу зі списку characters та округляємо до найбільшого цілого, яке менше.
  // Отримуємо символ з випадковим індексом.
  return password;// Додаємо отриманий випадковий символ до паролю.
}
...
function calculateCircleArea(radius) {
  if(isNaN(radius)){console.log("радіус не є числом!")}// Перевірка, чи переданий радіус є числом.
  else{return Math.pow(radius,2)*Math.PI}// Якщо радіус не є числом, виводимо в консоль повідомлення про помилку.
...
function findMinMax(numbers) {
  if(!Array.isArray(numbers)){console.log("переданий параметр не є масивом")}// Перевіряємо, чи переданий параметр є масивом.
  else{
...
function calculateHypotenuse(a, b) {
  if(isNaN(a)||isNaN(b)){console.log("довжини катетів не є числами")}// Перевіряємо, чи довжини катетів є числами. Оператор typeof повертає рядок, що вказує тип непустого операнда.
  else{const gepotenuz=Math.sqrt(Math.pow(a,2)+Math.pow(b,2));
  return gepotenuz}// Якщо довжини катетів не є числами, виводимо в консоль повідомлення про помилку.
  // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
...
function roundObjectValues(obj) {
  if(!typeof(obj)==='object'||(obj===null)){console.log("Помилка: аргумент має бути об'єктом"); return null;}// Перевіряємо, чи аргумент є об'єктом.
  else{let arr=Array();
    let i=0;
    for(const key in obj)
    {arr[i]=[key,obj[key]];
    i++};
    const map=arr.map(([k,el])=>(typeof(el)==='number')?el=[k,Math.round(el)]:[k,el]);
...
function calculateVolumeCylinder(radius, height) {
 if(isNaN(radius)||isNaN(height)) {console.log('один з аргументів не є числом'); return null}// Перевіряємо, чи є радіус і висота числами. Якщо хоча б один з аргументів не є числом, виводимо повідомлення про помилку в консоль.
  else{// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  const v=Math.PI*Math.pow(radius,2)*height;// Обчислюємо об'єм циліндра за формулою V = PI * r^2 * h, де PI - число Пі, r - радіус, h - висота.
 const rez=Math.ceil(v); // Округляємо об'єму до найменшого цілого числа числа що більше.
 return rez;} // Повертаємо обчислений об'єм.
}
...
function sumPositiveNumbers(numbers) {
  ...
  return null;}// Перевіряємо, чи є numbers масивом. Якщо numbers не є масивом, виводимо повідомлення "Помилка: аргумент має бути масивом чисел".
  else{// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  ...
  if(Math.sign(numbers[i])===1){
    sum=sum+numbers[i]}}// Перевіряємо, чи є поточне число додатним, використовуючи Math.sign.
  return sum;// Якщо число додатнє, додаємо його до суми.
...
function getFractionalPart(num) {
  if(isNaN(num)){console.log("Помилка: вхідний аргумент має бути числом."); return null}// Перевіряємо, чи є num числом. Якщо num не є числом, виводимо повідомлення "Помилка: вхідний аргумент має бути числом.".
  else{// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  let a=Math.trunc(num);// Округляємо відкидуючи дробову частину.
  a=num-a;// Віднімаємо цілу частину від початкового числа, щоб отримати дробову частину.
 a=Math.fround(a); // Округлюємо дробову частину з формату double до float.
  return a;}// Повертаємо дробову частину числа.
}
...
function compareAndRound(num1, num2) {
  if(isNaN(num1)||isNaN(num2)){console.log("Помилка: обидва аргументи мають бути числами.");
  return null}// Перевірка, чи обидва аргументи є числами. Якщо хоча б один аргумент не є числом, виводимо повідомлення "Помилка: обидва аргументи мають бути числами.".
  else{let max=Math.max(num1,num2);
  max=Math.round(max)
return max}// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
...
function estimateInvestment(principal, interestRate, years) {
  if(isNaN(principal)||isNaN(interestRate)||isNaN(years)){
    console.log("Помилка: обидва аргументи мають бути числами."); return null;
  }// Перевірка, чи усі аргументи є числами. Якщо хоча б один аргумент не є числом, виводимо повідомлення про помилку в консоль.
  else{// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  let sumInvest=principal*Math.pow((1+interestRate),years); // Розраховуємо суму інвестицій за формулою P*(1+rate)^years, де P - початкова сума інвестицій, rate - річна процентна ставка, years - кількість років.
  sumInvest=Math.round(sumInvest);// Округляємо ії до найближчого цілого
  return sumInvest;}// Повертаємо розраховану суму інвестицій.
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  console.log(" аргумент products не є масивом");// Якщо products не є масивом, виводимо повідомлення про помилку.
  return null}// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  else if(isNaN(maxPrice)){// Перевіряємо, чи аргумент maxPrice є числом.
  console.log("maxPrice не є числом");// Якщо maxPrice не є числом, виводимо повідомлення про помилку.
  return null;}// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  else{ let totalPrice=products.reduce((total,amout)=>total+amout.price);// Використовуємо метод reduce() для обчислення загальної ціни.
  totalPrice=Math.fround(totalPrice);
  maxPrice=Math.fround(maxPrice);// Додаємо ціну продукту до аккумулятора.
  if(totalPrice >maxPrice){console.log("totalPrice перевищує maxPrice"); return true}// Конвертуємо totalPrice та maxPrice за допомогою Math.fround.
  else{console.log("totalPrice не перевищує maxPrice"); return false}}// Порівнюємо, чи не перевищує totalPrice maxPrice.
}
